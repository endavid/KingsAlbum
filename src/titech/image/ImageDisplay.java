 ...
 * using this widget.
 *
 * <David> I slightly modified this class to also be able to see float
 *         images. ABSOLUTE, RESCALE or INVERSE are different ways to
 *         to rescale those values into byte values.
 *
 * @author Daniel Rice
 * @author Dennis Sigel
 * @author David Gavilan (modifications on float images, etc)
 */
public class ImageDisplay
    ...
    /** Visualization Tips */
    protected int visualizationMode = ABSOLUTE;
    ...
    /** This function is needed to change image datatype and also change sample values,
      * so they belong to range [0,255] and TYPE_BYTE.
      */
    private PlanarImage preprocess(PlanarImage im) {

        if (im.getSampleModel().getDataType() == DataBuffer.TYPE_BYTE) {
        ...
        switch (visualizationMode) {
            ...
            case RESCALE:
                prep = COps.rescale(im, 128f, 0.5f);
        ...
        return COps.reformat(prep, DataBuffer.TYPE_BYTE);
    }
                ...
                if (tile != null) {
                    ...
                    if (sampleModel.getDataType() == DataBuffer.TYPE_BYTE) {
                        ...
                        WritableRaster wr = Raster.createWritableRaster(
                                                    ...
                                                    null);
                        bi = new BufferedImage(colorModel, wr, 
                                               colorModel.isAlphaPremultiplied(), 
                                               null);
                    } else {
                        bi = new BufferedImage(tileWidth, tileHeight, 
                                               BufferedImage.TYPE_INT_RGB);
                        System.out.println(
                                "ImageDisplay: DataType not supported");
                    }

                    AffineTransform transform;
                    transform = AffineTransform.getTranslateInstance(
                                        tx + transX + insets.left, 
